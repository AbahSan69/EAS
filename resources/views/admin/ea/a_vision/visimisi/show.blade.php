...
<main id="main-container" class="flex-grow-1">
  ...
  <div class="content">
          <form action="{{ route('admin.ea.vision.visimisi.show', $id_pts) }}" method="GET">
              <div class="input-group mb-4">
                <input type="text" class="form-control" name="search" placeholder="Cari Visi Misi" value="{{ request()->input('search') }}">
                    ...
                    @foreach ($vision as $list_visimisi)
                      <tbody>
                        <tr>
                          ...
                          <td>
                            <button type="button" class="btn btn-sm btn-alt-warning js-bs-tooltip-enabled me-2" title="komentar" data-bs-toggle="modal" data-bs-target="#modal-block-vcenter-extra-large-komentar-{{ $list_visimisi->id }}">
                          ...
                          <td class="text-center">
                            <div class="btn-group">
                              <button type="button" class="btn btn-sm btn-alt-warning js-bs-tooltip-enabled me-2" title="Edit" data-bs-toggle="modal" data-bs-target="#modal-block-vcenter-extra-large-tambah-{{ $list_visimisi->id }}">
                                  <i class="fa fa-fw fa-pencil"></i>
                              </button>
                              <button type="button" class="btn btn-sm btn-alt-danger js-bs-tooltip-enabled" data-bs-toggle="tooltip" aria-label="Delete" data-bs-original-title="Delete" title="Hapus" onclick="confirmationHapusData('{{ route('admin.ea.vision.visimisi.delete', $list_visimisi->id) }}')">
                      ...
                      <!-- modal Edit Data -->
                      <div class="modal fade" id="modal-block-vcenter-extra-large-tambah-{{ $list_visimisi->id }}" tabindex="-1" aria-labelledby="modal-block-vcenter" style="display: none;" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            <div class="block block-rounded block-transparent mb-0">
                              <div class="block-header block-header-default">
                              ...
                              <div class="block-content fs-sm">
                                <form class="space-y-4" action="{{ route('admin.ea.vision.visimisi.update') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                  ...
                                  <div class="row">
                                    <div class="mb-4">
                                      ...
                                      <input type="text" class="form-control" id="title" name="title" title="title"  placeholder="Masukan judul ..." value="{{ $list_visimisi->title }}" required>
                                    </div>
                                    <div class="mb-4">
                                      <label class="form-label" for="val-suggestions">Konten</span></label>
                                      <textarea class="form-control" id="content" name="content" rows="5" placeholder="content ...">{{ $list_visimisi->content }}</textarea>
                                    </div>
                                    <div class="mb-4">
                                      ...
                                      <input type="file" class="form-control" id="image" name="image" accept=".jpg,.jpeg,.png">
                                        ...
                                        <div class="mt-3">
                                          ...
                                          <img src="{{ asset($list_visimisi->latestHistory->image) }}" alt="Gambar Lama" class="img-fluid rounded" style="max-height: 200px;">
                                        </div>
                                        @endif
                                    </div>
                                    <div class="mb-4">
                                      ...
                                      <select class="form-select" id="status" name="status" required>
                                        ...
                                        <option value="Proses" {{ old('status', $list_visimisi->latestHistory->status ?? '') == 'Proses' ? 'selected' : '' }}>Proses</option>
                                        <option value="Selesai" {{ old('status', $list_visimisi->latestHistory->status ?? '') == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                                      </select>
                                    </div>
                                  </div>
                              </div>
      ...
      <div class="modal fade" id="modal-block-vcenter-extra-large-tambah" tabindex="-1" aria-labelledby="modal-block-vcenter" style="display: none;" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              <div class="block block-rounded block-transparent mb-0">
                <div class="block-header block-header-default">
                  <h3 class="block-title">Modal Tambah visimisi</h3>
                  <div class="block-options">
                    <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                      <i class="fa fa-fw fa-times"></i>
                    </button>
                ...
                <div class="block-content fs-sm">
                  <form class="space-y-4" action="{{ route('admin.ea.vision.visimisi.save') }}" method="POST" enctype="multipart/form-data">
                    @csrf
