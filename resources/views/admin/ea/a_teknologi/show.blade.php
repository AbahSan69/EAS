...
<main id="main-container" class="flex-grow-1">
  ...
  <div class="content">
          <form action="{{ route('admin.ea.teknologi.show', $id_pts) }}" method="GET">
              <div class="input-group mb-4">
                <input type="text" class="form-control" name="search" placeholder="Cari Arsiktektur Teknologi" value="{{ request()->input('search') }}">
          ...
          <div class="block block-rounded">
              <div class="block-header block-header-default">
                <h3 class="block-title">List teknologi</h3>
                ...
                <div class="table-responsive">
                  <table class="table table-vcenter">
                    ...
                    @php
                      $no = 0;
                    @endphp
                    @foreach ($teknologi as $list_teknologi)
                      <tbody>
                        <tr>
                            ...
                            <td>
                              @if($list_teknologi->latestHistory->image)
                                <div class="image-preview" data-bs-toggle="modal" data-bs-target="#imageModal-{{ $list_teknologi->id }}">
                                  ...
                                  width="80" 
                                  class="img-thumbnail"> --}}
                                  <img src="{{ asset($list_teknologi->latestHistory->image) }}" 
                                  alt="gambar" 
                                  width="80" 
                                  class="img-thumbnail">
                          ...
                          <td>
                            <button type="button" class="btn btn-sm btn-alt-warning js-bs-tooltip-enabled me-2" title="komentar" data-bs-toggle="modal" data-bs-target="#modal-block-vcenter-extra-large-komentar-{{ $list_teknologi->id }}">
                          ...
                          <td class="text-center">
                            <div class="btn-group">
                              <button type="button" class="btn btn-sm btn-alt-warning js-bs-tooltip-enabled me-2" title="Edit" data-bs-toggle="modal" data-bs-target="#modal-block-vcenter-extra-large-tambah-{{ $list_teknologi->id }}">
                                  <i class="fa fa-fw fa-pencil"></i>
                              </button>
                              <button type="button" class="btn btn-sm btn-alt-danger js-bs-tooltip-enabled" data-bs-toggle="tooltip" aria-label="Delete" data-bs-original-title="Delete" title="Hapus" onclick="confirmationHapusData('{{ route('admin.ea.teknologi.delete', $list_teknologi->id) }}')">
                      ...
                      <!-- Modal Gambar -->
                      <div class="modal fade" id="imageModal-{{ $list_teknologi->id }}" tateknologindex="-1">
                      ...
                      <!-- modal komentar -->
                      <div class="modal fade" id="modal-block-vcenter-extra-large-komentar-{{ $list_teknologi->id }}" tabindex="-1" aria-labelledby="modal-block-vcenter" style="display: none;" aria-hidden="true">
                            ...
                            <div class="block block-rounded block-transparent mb-0">
                              ...
                              <div class="block-content fs-sm">
                                <div class="mb-3">
                                  ...
                                  @forelse($list_teknologi->teknologi_comments as $cmt)
                      ...
                      <!-- modal Edit Data -->
                      <div class="modal fade" id="modal-block-vcenter-extra-large-tambah-{{ $list_teknologi->id }}" tabindex="-1" aria-labelledby="modal-block-vcenter" style="display: none;" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="bicument">
                          <div class="modal-content">
                            <div class="block block-rounded block-transparent mb-0">
                              <div class="block-header block-header-default">
                                <h3 class="block-title">Modal Edit teknologi</h3>
                                <div class="block-options">
                                  <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                                    <i class="fa fa-fw fa-times"></i>
                                  </button>
                              ...
                              <div class="block-content fs-sm">
                                <form class="space-y-4" action="{{ route('admin.ea.teknologi.update') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                  <input type="hidden" name="id" id="id" value="{{ $list_teknologi->id }}">
                                  <div class="row">
                                    <div class="mb-4">
                                      <label class="col-sm-4 col-form-label" for="example-hf-title">Judul <span class="text-danger">*</span></label>
                                      <input type="text" class="form-control" id="title" name="title" title="title"  placeholder="Masukan judul ..." value="{{ $list_teknologi->title }}" required>
                                    </div>
                                    <div class="mb-4">
                                      <label class="form-label" for="val-suggestions">Konten</span></label>
                                      <textarea class="form-control" id="content" name="content" rows="5" placeholder="content ...">{{ $list_teknologi->content }}</textarea>
                                    </div>
                                    <div class="mb-4">
                                      ...
                                      <input type="file" class="form-control" id="image" name="image" accept=".jpg,.jpeg,.png">
                                        ...
                                        <div class="mt-3">
                                          ...
                                          <img src="{{ asset($list_teknologi->latestHistory->image) }}" alt="Gambar Lama" class="img-fluid rounded" style="max-height: 200px;">
                                        </div>
                                        @endif
                                    </div>
                                    <div class="mb-4">
                                      ...
                                      <select class="form-select" id="status" name="status" required>
                                        ...
                                        <option value="Proses" {{ old('status', $list_teknologi->latestHistory->status ?? '') == 'Proses' ? 'selected' : '' }}>Proses</option>
                                        <option value="Selesai" {{ old('status', $list_teknologi->latestHistory->status ?? '') == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                                      </select>
                                    </div>
                                  </div>
                              </div>
      ...
      <div class="modal fade" id="modal-block-vcenter-extra-large-tambah" tabindex="-1" aria-labelledby="modal-block-vcenter" style="display: none;" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="bicument">
            <div class="modal-content">
              <div class="block block-rounded block-transparent mb-0">
                <div class="block-header block-header-default">
                  <h3 class="block-title">Modal Tambah teknologi</h3>
                  <div class="block-options">
                    <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                      <i class="fa fa-fw fa-times"></i>
                    </button>
                ...
                <div class="block-content fs-sm">
                  <form class="space-y-4" action="{{ route('admin.ea.teknologi.save') }}" method="POST" enctype="multipart/form-data">
                    @csrf
