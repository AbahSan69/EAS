...
namespace App\Http\Controllers\StakeholderPTS\EA\AInformasi;
...
class InformasiController extends Controller
{
    public function show($id, Request $request)
    {
        ...
        return view('stakeholder_pts.ea.a_informasi.show', compact('id_pts', 'informasi'));
    }
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            // simpan ke folder informasi_images/{id_pts}
            //     $imagePath = $request->file('image')
            //         ->store("informasi_images/{$request->id_pts}", 'public');
            // }
            if ($request->hasFile('image')) {
                $folder = public_path("informasi_images/{$request->id_pts}/{$informasi->id}");
                ...
                $imagePath = "informasi_images/{$request->id_pts}/{$informasi->id}/{$fileName}";
            }

            // ✅ Simpan ke history
            SPinformasiHistory::create([
                'sp_informasi_id' => $informasi->id,
    ...
    {
        $request->validate([
            'id'      => 'required|integer|exists:sp_architecture_informasi,id',
            'title'   => 'required|string|max:255',
            'status'  => 'required|in:Proses,Selesai',
            'content' => 'nullable|string',
        ...
        try {
            $informasi = SPinformasi::findOrFail($request->id);
    ...
    public function delete($id)
    {
        ...
        try {
            $informasi = SPinformasi::findOrFail($id);

            // ✅ Hapus semua file gambar dari setiap history
            foreach ($informasi->histories as $history) {
                if ($history->image && file_exists(public_path($history->image))) {
                    @unlink(public_path($history->image));
                }
            }

            // ✅ Hapus folder informasi_images/{pts_id}/{informasi_id}
            $folder = public_path("informasi_images/{$informasi->pts_id}/{$informasi->id}");
